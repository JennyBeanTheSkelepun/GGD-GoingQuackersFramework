...
struct ImFontAtlas;                 // Runtime data for multiple fonts, bake multiple fonts into a single mp_texture, TTF/OTF font loader
...
typedef void* ImTextureID;          // User data for rendering backend to identify a mp_texture. This is whatever to you want it to be! read the FAQ about ImTextureID for details.
#endif
...
struct ImGuiIO
{
    ...
    ImFontAtlas*Fonts;                          // <auto>           // Font atlas: load, rasterize and pack one or more fonts into a single mp_texture.
...
//  B) render a complex 3D scene inside a UI element without an intermediate mp_texture/render target, etc.
...
struct ImDrawCmd
{
    ...
    ImTextureID     TextureId;          // 4-8  // User-provided mp_texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.
...
struct ImDrawList
{
    ...
    // - "uv_min" and "uv_max" represent the normalized mp_texture coordinates to use for those corners. Using (0,0)->(1,1) mp_texture coordinates will generally display the entire mp_texture.
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoMouseCursors     = 1 << 1,   // Don't build software mouse cursors into the atlas (save a little mp_texture memory)
    ImFontAtlasFlags_NoBakedLines       = 1 << 2    // Don't build thick line textures into the atlas (save a little mp_texture memory). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};

// Load and rasterize multiple TTF/OTF fonts into a same mp_texture. The font atlas will build a single mp_texture holding:
//  - One or more fonts.
//  - Custom graphics data needed to render the shapes needed by Dear ImGui.
//  - Mouse cursor shapes for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
// It is the user-code responsibility to setup/build the atlas, then upload the pixel data into a mp_texture accessible by your graphics api.
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the code will be loaded for you.
//  - Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels data.
//  - Upload the pixels data into a mp_texture within your graphics system (see imgui_impl_xxxx.cpp examples)
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your mp_texture in a format natural to your graphics API.
//    This value will be passed back to you during rendering to identify the mp_texture. Read FAQ entry about ImTextureID for more details.
// Common pitfalls:
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearInputData();           // Clear input data (all ImFontConfig structures including sizes, TTF data, glyph ranges, etc.) = all the data used to build the mp_texture and fonts.
    IMGUI_API void              ClearTexData();             // Clear output mp_texture data (CPU side). Saves RAM once the mp_texture has been copied to graphics memory.
    IMGUI_API void              ClearFonts();               // Clear output font data (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // Clear all input and output.

    // Build atlas, retrieve pixel data.
    // User is in charge of copying the pixels into graphics memory (e.g. create a mp_texture with your engine). Then store your mp_texture handle with SetTexID().
    // The pitch is always = Width * BytesPerPixels (1 or 4)
    // Building in RGBA32 format is provided for convenience and compatibility, but note that unless you manually manipulate or copy color data into
    // the mp_texture (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted will always be white (~75% of memory/bandwidth waste.
    ...
    ImTextureID                 TexID;              // User data to refer to the mp_texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have mp_texture size restrictions you may want to increase mp_texture width to decrease height.
    int                         TexGlyphPadding;    // Padding between glyphs within mp_texture in pixels. Defaults to 1. If your rendering method doesn't rely on bilinear filtering you may set this to 0.

    // [Internal]
    // NB: Access mp_texture data via GetTexData*() calls! Which will setup a default font for you.
    ...
    ImVector<ImFontAtlasCustomRect> CustomRects;    // Rectangles for packing custom mp_texture data into the atlas.
    ...
    int                         PackIdMouseCursors; // Custom mp_texture rectangle ID for white pixel and mouse cursors
    int                         PackIdLines;        // Custom mp_texture rectangle ID for baked anti-aliased lines
...
struct ImFont
{
    ...
    int                         MetricsTotalSurface;// 4     // out //            // Total surface in pixels to get an idea of the font rasterization/mp_texture cost (not exact, we approximate the cost of padding between glyphs)
